...
public class MyCanvas extends JPanel
{
   ...
   public void updateBoard(boolean[][] grid)
   {
      ...
      for (int row = 0; row < grid.length; row++)
      {
         for (int col = 0; col < grid[0].length; col++)
         {
            ...
            if (isValidCell(grid, row-1, col-1))
            {
               if(grid[row-1][col-1])
               {
                  aliveCounter += 1;
            ...
            if (grid[row][col])
            {
               if (aliveCounter > 3 || aliveCounter < 2)
               {
   ...
   */
   public void drawBoard(Graphics2D g2, boolean[][] tempGrid)
   {
      int squareSize = 500 / grid.length;
      int x1 = 0;
      int y1 = 0;
      for (int row = 0; row < grid.length; row++)
      {
         for (int col = 0; col < grid[0].length; col++)
         {
            if (grid[row][col])
            {
               ...
               Rectangle2D.Double redSquare = new Rectangle2D.Double(x1, y1, squareSize, squareSize);
               g2.draw(redSquare);
               g2.fill(redSquare);
            }
               ...
               Rectangle2D.Double whiteSquare = new Rectangle2D.Double(x1, y1, squareSize, squareSize);
               g2.draw(whiteSquare);
            }
            x1 += squareSize;
         }
